        ...
        <div className="min-h-screen bg-gray-50">
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 text-center animate-in zoom-in-95">
                        ...
                        <div className="bg-gray-50 rounded-xl p-4 mb-6">
                            <div className="flex items-center justify-center gap-2 text-gray-600 mb-2">
                                ...
                                <span className="font-medium">SMS başarıyla gönderildi</span>
                            </div>
                            <p className="text-indigo-600 font-semibold">{customer.phone || 'Telefon numarası yok'}</p>
                        </div>
            ...
            <div className="sticky top-0 z-20 bg-white border-b border-gray-200 shadow-sm">
                <div className="max-w-[1920px] mx-auto px-6 py-5">
                    <div className="flex flex-col lg:flex-row items-start lg:justify-between gap-6 w-full">
                        {/* Left: Avatar + Info */}
                        ...
                        {(customer.user || customer.plainUsername || customer.plainPassword) && (
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3 sm:py-2 w-full lg:w-auto">
                                ...
                                <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="font-mono font-semibold text-gray-900">
                                        {customer.user?.plainUsername || customer.plainUsername || customer.user?.email || customer.email || '—'}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="font-mono font-bold text-indigo-600 text-sm">
                                        {customer.user?.plainPassword || customer.plainPassword || '—'}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2 mt-2 sm:mt-0 w-full sm:w-auto">
                                    <button
                                        onClick={() => {
                                            const username = customer.user?.plainUsername || customer.plainUsername || customer.user?.email || customer.email || '';
                                            const password = customer.user?.plainPassword || customer.plainPassword || 'Şifre kaydedilmemiş';
                                            const text = `Kullanıcı adı: ${username}\nŞifre: ${password}`;
                                            navigator.clipboard.writeText(text);
                                            showAlert('Giriş bilgileri kopyalandı!', 'success');
                                        }}
                                        className="flex-1 sm:flex-none flex items-center justify-center gap-1 px-3 py-1.5 sm:py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg sm:rounded text-xs font-medium transition-all sm:ml-2"
                                        title="Kopyala"
                                    >
                                    ...
                                    <button
                                        onClick={async () => {
                                            if (!confirm('Müşteriye giriş bilgileri SMS olarak gönderilecektir. Onaylıyor musunuz?')) return;
                                            
                                            try {
                                                const res = await fetch(`/api/customers/${customer._id}/send-credentials`, { method: 'POST' });
                                                const data = await res.json();
                                                if (res.ok && data.success) {
                                                    showAlert('SMS başarıyla gönderildi!', 'success');
                                                } else {
                                                    showAlert(data.error || 'SMS gönderilemedi', 'error');
                                                }
                                            } catch (err) {
                                                showAlert('Bağlantı hatası', 'error');
                                            }
                                        }}
                                        className="flex items-center gap-1 px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-medium transition-all ml-2 shadow-sm"
                                        title="SMS Gönder"
                                    >
                        ...
                        <div className="flex flex-col sm:flex-row items-start gap-4 w-full lg:w-auto">
                            ...
                            <div className="relative w-full sm:w-auto">
                                ...
                                <button
                                    ...
                                    className={`flex items-center w-full gap-2 px-4 py-2.5 rounded-lg text-white text-sm font-medium min-w-[180px] justify-between ${currentAppointmentDisplay.color} ${pendingAppointmentStatus ? 'ring-2 ring-amber-400 ring-offset-2' : ''}`}
                                >
                                    <div className="flex items-center gap-2">
                            ...
                            <div className="relative w-full sm:w-auto mt-2 sm:mt-0">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 w-full px-4 py-2.5 rounded-lg text-white text-sm font-medium min-w-[180px] justify-between ${currentAlbumDisplay.color} ${pendingAlbumStatus ? 'ring-2 ring-pink-400 ring-offset-2' : ''}`}
                                >
                                    <div className="flex items-center gap-2">
